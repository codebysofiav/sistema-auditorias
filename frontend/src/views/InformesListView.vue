...
async function cargar() {
  ...
  errorMsg.value = ''
  try {
    const [informesRes, auditoriasRes] = await Promise.all([
    ...
    errorMsg.value = 'No se pudo cargar el listado de informes.'
  } finally {
    loading.value = false
  }
}
...
<template>
  <div class="shell">
    ...
    <main class="main">
      ...
      <p v-else-if="informes.length === 0" class="empty-note">
        No hay informes para mostrar todavía.
      </p>
