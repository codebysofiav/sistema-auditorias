...
async function cargar() {
  ...
  errorMsg.value = ''
  try {
    const [documentosRes, auditoriasRes] = await Promise.all([
      apiClient.get('/documentos/'),
      apiClient.get('/auditorias/'),
    ])
    documentos.value = documentosRes.data.results ?? documentosRes.data
    ...
    errorMsg.value = 'No se pudo cargar el listado de documentos.'
  } finally {
    loading.value = false
  }
}
    ...
    <main class="main">
      <div class="header-row">
        <div>
          ...
          <p class="sub">Historial de documentos oficiales generados a partir de las plantillas de la universidad.</p>
        </div>
        <button v-if="puedeEscribir" class="btn-primary" @click="router.push({ name: 'documento-generar' })">
          + Generar documento
        </button>
      </div>
      ...
      <p v-else-if="documentos.length === 0" class="empty-note">
        No se ha generado ningún documento todavía.
      </p>
      ...
      <table v-else>
        ...
        <tbody>
          <tr v-for="d in documentos" :key="d.id">
            <td class="code">{{ auditoriasPorId[d.auditoria] ?? d.auditoria }}</td>
            <td>{{ d.tipo_documento }}</td>
            <td class="code">{{ d.formato }}</td>
            <td class="code">{{ d.fecha_generacion }}</td>
            <td class="actions-cell">
              <a v-if="d.archivo" :href="d.archivo" class="btn-link" target="_blank" rel="noopener">Descargar</a>
              <span v-else class="pendiente">Sin archivo</span>
            </td>
          </tr>
        </tbody>
      </table>
...
.sub { font-size: 13px; color: var(--ink-soft); margin: 0; max-width: 480px; }
...
.btn-link { background: none; border: 1px solid var(--line); border-radius: var(--radius); padding: 5px 10px; font-size: 12px; cursor: pointer; color: var(--ink); text-decoration: none; }
.pendiente { font-size: 11px; color: var(--ink-soft); font-style: italic; }
